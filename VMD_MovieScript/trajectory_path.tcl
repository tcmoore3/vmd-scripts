...
proc trajectory_path {selection {color black} {linewidth 1} {update 1}} {
 ...
 # 'frame' in the next line used to be numframes
 set num_frames [molinfo $gr_mol get frame] 
 set coords {}
 ...
 if {$color == "scale"} {
   ...
   graphics $gr_mol color $color
   foreach coord $coords {
     lappend gr_list [graphics $gr_mol line $prev $coord style dashed width $linewidth]
     set prev $coord
   }
 }
